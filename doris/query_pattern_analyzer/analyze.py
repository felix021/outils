...
def normalize_query(query):
    ...
    # print('%s: %s' % (type(query), str(query)))
...
def aggregate_queries(file_path):
    ...
    df['query_pattern'] = df['stmt'].parallel_apply(normalize_query)
    #df['query_pattern'] = df['stmt'].apply(normalize_query)
    ...
    aggregation = df.groupby('query_pattern').agg({
        ...
        'query_pattern': 'execution_count',
        'stmt': 'query_sample',
    }).reset_index()

    # Sort by the most resource-consuming queries
    aggregation = aggregation.sort_values(by=['cpu_time_second'], ascending=False)
